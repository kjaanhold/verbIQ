...
def return_test_score(name):
    ...
    if TestResults.query.filter_by(lapse_eesnimi = name).first() is None:
      ...
      bottom_skills_data = TestResults.query.filter_by(lapse_eesnimi = name, result_cdf_value = 'Ei').order_by(TestResults.result_cdf_value.asc()).limit(3)
      ...
      out_text = str(result_cdf_value) + "///" + str(bottom_block_name)
    return str(out_text)
