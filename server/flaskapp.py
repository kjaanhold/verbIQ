...
#    name = request.args.get('Lapse_eesnimi')
    ...
    if not TestResults.query.filter_by(lapse_eesnimi = name.lower()).first():
      ...
      query = "SELECT t.description, t.block_name, t.id_test FROM tests t JOIN milestone_tests ms ON t.id_test = ms.key_test JOIN milestones m ON ms.key_milestone = m.id_milestone WHERE m.target_age <= (4*%s)/3 AND m.target_age >= 23 AND t.block_name NOT IN (%s) ORDER BY RANDOM() LIMIT 1;" % (age_months, age_months, block_name)
      rows = execute_query(query)
