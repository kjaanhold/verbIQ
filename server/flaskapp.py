...
#def return_test_results(name, result_value):
    ...
    if TestResults.query.filter_by(lapse_eesnimi = name.lower()).first() is None:
      ...
      query = "SELECT m.target_month, t.block_name FROM tests t JOIN milestone_tests ms ON t.id_test = ms.key_test JOIN milestones m ON ms.key_milestone = m.id_milestone WHERE t.block_name IN (%s);" % (block_name)    
#      rows = execute_query(query)
