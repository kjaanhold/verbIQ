...
def propose_exercise():
    ...
    query = "SELECT group_concat(m.description) FROM tests t JOIN milestone_tests ms ON t.id_test = ms.key_test JOIN milestones m ON ms.key_milestone = m.id_milestone JOIN milestones_exercises me ON ms.key_milestone = me.key_milestone JOIN exercises e ON me.key_exercise = e.id_exercise WHERE t.block_name IN (%s) LIMIT 3;" % (bottom_block_name)    



    rows = execute_query(query)

    return str(rows[0][0].encode("utf-8"))
'''
