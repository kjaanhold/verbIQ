...
def next_test_selection():
    ...
    if TestResults.query.filter_by(lapse_eesnimi = name.lower()).first() is None:
      ...
      query = "SELECT t.description, t.block_name FROM tests t JOIN milestone_tests ms ON t.id_test = ms.key_test JOIN milestones m ON ms.key_milestone = m.id_milestone ORDER BY RANDOM() LIMIT 1;"
